 ...
  * Allocates inventory from batches received earliest 

  */

 @Component

 public class FIFOStrategy implements InventoryStrategy {

   @Override

   public List < Batch > allocate(List < Batch > batches, int quantityNeeded) {

     // Sort batches by received date (ascending - earliest received first) 

     List < Batch > sortedBatches = new ArrayList < > (batches);

     sortedBatches.sort(Comparator.comparing(Batch::getReceivedDate));
